...
import 'package:system_metrics_widget/src/widgets/metrics/display_info_widget.dart';
...
class SystemInfoMetrics extends StatelessWidget {
  ...
  const SystemInfoMetrics({required SystemInfo model, super.key})
    : _model = model;
      ...
      children: <Widget>[
        ...
        StaggeredGridTile.fit(
          ...
          child: DiskUsageWidget(
            totalMemory:
                _model.updateInfo.diskUsed + _model.updateInfo.diskAvailable,
        ...
        if (_model.updateInfo.sensors.isNotEmpty)
          StaggeredGridTile.fit(
            crossAxisCellCount: 4,
            child: SensorsInfoWidget(model: _model.updateInfo),
          ),
      ],
    );
  }
