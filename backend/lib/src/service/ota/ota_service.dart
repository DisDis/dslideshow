...
class OTAService implements RpcService {
  ...
  OTAInfo _info = OTAInfo(status: OTAStatus.disabled, code: "-", uploadingPercent: 0);
  ...
  Future<RpcResult> _executeEchoCommand(EchoCommand command) async {
    ...
    return new EchoCommandResult(id: command.id, resultText: "${command.text} Service ${new DateTime.now()}");
  }
  ...
  Future<Response> _postOTAUploadPackage(Request request) async {
    ...
    _updateInfo(_info.copyWith(uploadingPercent: 0, status: OTAStatus.uploading));
...
""";
  ...
  void _processFirmware(String filename, io.BytesBuilder firmwareData) async {
    ...
    _frontendService.send(OTAOutputCommand(id: RpcCommand.generateId(), output: '\n\rExit code: $exitCode'));
    _updateInfo(_info.copyWith(uploadingPercent: 100, exitCode: exitCode, status: exitCode == 0 ? OTAStatus.finished : OTAStatus.issue));
    if (exitCode == 0) {
      Timer(const Duration(seconds: 5), () {
        _updateInfo(_info.copyWith(uploadingPercent: 100, exitCode: 0, status: OTAStatus.preReboot));
        Timer(const Duration(seconds: 5), () {
